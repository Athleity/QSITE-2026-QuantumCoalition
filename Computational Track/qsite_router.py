...
class Campaign:
    ...
    def commit(self, layout, node, tag=""):
    ...
    def run(self, deadline):
        ...
        end1 = time.perf_counter() + 0.5 * (deadline - time.perf_counter())
        i = 0
        while time.perf_counter() < end1 and not self.finished(deadline):
            lay = starts[i] if i < len(starts) else self.fresh_layout()
            i += 1
            self.chain(lay, deadline, 16, noise=rng.choice((0.0, 0.0, 0.3)))
        # stage 2: wider chains on the best layouts
        end2 = time.perf_counter() + 0.6 * (deadline - time.perf_counter())
        for w in (64, 192):
            for cost, lay in list(self.pool)[:4]:
                if time.perf_counter() > end2 or self.finished(deadline):
                    break
                self.chain(list(lay), deadline, w, rounds=6)
        # stage 3: noisy, re-parameterised chains around the elites until the clock runs out
        while not self.finished(deadline):
            if not self.pool:
                self.chain(self.fresh_layout(), deadline, 16)
                continue
